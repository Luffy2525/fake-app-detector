"""
Fake App Detector — Backend (v1.2.0)
...
MAX_REPORTS = int(os.environ.get('MAX_REPORTS', 500))      # NEW: cap stored reports
...
def extract_permissions_from_axml(data: bytes) -> list:
    ...
    try:
        ...
        chunk_size    = struct.unpack_from('<I', data, offset + 4)[0]   # noqa: F841
        string_count  = struct.unpack_from('<I', data, offset + 8)[0]
        style_count   = struct.unpack_from('<I', data, offset + 12)[0]  # noqa: F841
...
def analyze_apk():
    ...
    app_name    = request.form.get('app_name', file.filename.rsplit('.', 1)[0]).strip()
    developer   = request.form.get('developer', 'Unknown Developer').strip()
    description = request.form.get('description', '').strip()

    # BUG FIX 7: Sanitise app_name/developer to avoid log injection
    app_name  = re.sub(r'[\r\n\t]', ' ', app_name)[:128]
    developer = re.sub(r'[\r\n\t]', ' ', developer)[:128]
    ...
    try:
        ...
        report = {
            ...
            "module1_preprocessing": {
                ...
                # Surface simulation warning to the frontend
...
# BUG FIX 8: /api/analyze-url now validates URL format and is clearly labelled
#            as metadata-only analysis (no actual HTTP fetch of the APK).
@app.route('/api/analyze-url', methods=['POST'])
def analyze_url():
    t0   = time.time()
    data = request.get_json() or {}

    app_name    = re.sub(r'[\r\n\t]', ' ', data.get('app_name', 'Unknown App')).strip()[:128]
    developer   = re.sub(r'[\r\n\t]', ' ', data.get('developer', 'Unknown')).strip()[:128]
    ...
    # BUG FIX 10: Pass fingerprint string to fake detector — it can never match
    #             KNOWN_MALICIOUS_HASHES (which stores real APK md5s), which is correct
    #             because we have no real file here.
    ...
    return jsonify({
        "meta": {
            ...
            "api_version":        "1.2.0",
            # Warn consumers that permissions here are estimated
            "metadata_warning":   (
...
if __name__ == '__main__':
    ...
    logger.info("Starting Fake App Detector on port %d (debug=%s)", port, debug)
